...
class CustomerFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        if not filter_string.isdigit():
            return data
        for c in customers:
            id_collection.append(c.get_id())
            if str(c.get_id()) == filter_string:
                filtered_calls.extend(c.get_history()[0])
                filtered_calls.extend(c.get_history()[1])
        if int(filter_string) not in id_collection:
            return data
        return filtered_calls
        """
        target = None
        ...
        for cust in customers:
            if filter_string == str(cust.get_id()):
                target = cust
        if not target :
            return data
        for call in data:
            if call.dst_number in target.get_phone_numbers() or call.src_number\
                    in target.get_phone_numbers():
...
class DurationFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        if not filter_string:
            return data
        elif filter_string[0].isupper:
            if not filter_string[1:].isdigit():
                return data
            d = int(filter_string[1:])
            if filter_string[0] == 'L':
                for c in data:
                    if c.duration < d:
                        filtered_calls.extend([c])
...
class LocationFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        for num in coordinates:
            if not (num[:1].replace("-", "") + num[1:]).replace(".", "", 1).\
                    isdigit():
                return data
        for x in data:
            if float(coordinates[2]) > x.src_loc[0] > float(coordinates[0]) and\
                    float(coordinates[3]) > x.src_loc[1] >\
                    float(coordinates[1]) or float(coordinates[2]) >\
                    x.dst_loc[0] > float(coordinates[0]) and\
                    float(coordinates[3]) > x.dst_loc[1] >\
                    float(coordinates[1]):
                filtered_calls.extend([x])
        return filtered_calls
